...
func digestPath(resource string) (string, error) {
	...
	path := filepath.Join(u.HomeDir, ".kb", "digest", name)
	return path, nil
}
...
type BuildOptions struct {
	...
	Force      bool     `json:"force,omitempty"`     // If true, will always run docker build regardless of kb digest
...
func locateSpec(file string) (string, error) {
	...
	return filepath.Join(dir, "kb.yaml"), nil
}
