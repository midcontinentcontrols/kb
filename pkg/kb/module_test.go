...
CMD ["sh", "-c", "echo \"Hello, world\""]`
			...
			p := NewProcess(runtime.NumCPU(), logger.NewFakeLogger())
			module, err := p.GetModule(filepath.Join(rootPath, "dir", "kb.yaml"))
			require.NoError(t, err)
			err = module.Build(&BuildOptions{NoPush: true})
			require.Error(t, err)
...
CMD ["sh", "-c", "echo \"Hello, world\""]`
			...
			p := NewProcess(runtime.NumCPU(), logger.NewFakeLogger())
			module, err := p.GetModule(filepath.Join(rootPath, "subdir", "kb.yaml"))
			require.NoError(t, err)
			require.NoError(t, module.Build(&BuildOptions{NoPush: true}))
			require.Equal(t, BuildStatusSucceeded, module.Status())
		})
...
CMD ["sh", "-c", "echo \"foo bar baz\""]`, name)
			...
			log = logger.NewMockLogger(logger.NewFakeLogger())
			module, err = NewProcess(runtime.NumCPU(), log).GetModule(filepath.Join(rootPath, "dep", "kb.yaml"))
			require.NoError(t, err)
			require.Equal(t, BuildStatusPending, module.Status())
			require.NoError(t, module.Build(&BuildOptions{NoPush: true}))
			require.Equal(t, BuildStatusSucceeded, module.Status())
...
build:
  name: %s`,
			...
			depDockerfile := `FROM alpine:3.11.6
CMD ["sh", "-c", "echo \"Hello, world\""]`
			dockerfile := fmt.Sprintf(`ARG KB_TAG
FROM %s-dep:${KB_TAG}
COPY foo foo
...
// This ensures the buildArgs: section of kb.yaml is properly applied
